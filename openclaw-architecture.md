...
**UID Mismatch Workaround:** The `ocl` host user is uid 1001, but the gateway pod's `node` user is uid 1000. The credentials file must be `chmod 666` (all rw) for the pod to both read AND write it. Write access is critical: when token-sync.js refreshes the token, Anthropic rotates the refresh token — if the new refresh token can't be persisted to disk, the on-disk copy becomes permanently invalid (`invalid_grant`). Both `token-sync.js` and the CronJob write with `{ mode: 0o666 }` to preserve cross-UID access. The wizard sets this during `deploy_gateway_pod()`.
