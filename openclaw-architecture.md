...
**Anthropic OAuth Token Lifecycle:** Anthropic OAuth tokens have ~8h TTL. **Claude Code CLI is the sole token refresher** — no other process attempts to refresh. This eliminates the race condition where multiple processes using single-use refresh tokens simultaneously caused `invalid_grant` loops (Anthropic rotates the refresh token on each use; the second caller always fails).
1. **Refresher (CLI only):** Claude Code CLI on the host auto-refreshes its OAuth token before expiry and writes to `~/.claude/.credentials.json`. This is the single source of truth.
2. **Distributor (60s latency):** `token-sync.js` runs as a read-only background process inside the gateway pod. Every 60 seconds it reads `credentials.json` (hostPath directory mount) and distributes the token to `auth-profiles.json` for all 13 agents. It does NOT refresh — only reads and distributes. If the token is expired, it logs a warning once per 10 minutes: "run /login in Claude Code CLI". openclaw reads `auth-profiles.json` from disk on every API call (no cache), so token changes take effect immediately.
3. **Secret sync (30m):** `anthropic-oauth-refresh` CronJob runs every 30 minutes. Read-only: reads `credentials.json` and syncs token values to k8s Secret `anthropic-oauth` (for pod restart bootstrap). Does NOT refresh.
4. **Bootstrap:** On pod start, the startup script reads `ANTHROPIC_OAUTH_TOKEN`, `ANTHROPIC_REFRESH_TOKEN`, and `ANTHROPIC_OAUTH_EXPIRES` from the k8s Secret (injected as env vars), writes them to `auth-profiles.json` for each agent, then launches token-sync.js.

**UID Mismatch Workaround:** The `ocl` host user is uid 1001, but the gateway pod's `node` user is uid 1000. The credentials file must be `chmod 666` (all rw) for the pod to read it. The wizard sets this during `deploy_gateway_pod()`, and a host crontab (`* * * * * chmod 666 ~/.claude/.credentials.json`) counteracts Claude Code CLI resetting permissions to 600 on writes.

**Directory Mount (Stale-Inode Fix):** The credentials volume uses a **directory mount** (`~/.claude/` → `/creds/`) instead of a single-file mount. When Kubernetes bind-mounts a single file (`type: File`), it pins the inode at mount time. If the host replaces the file (new inode — e.g., Claude Code CLI refreshing its token), the pod continues reading the old deleted inode (`nlink: 0`) indefinitely. With a directory mount, each `readFileSync('/creds/.credentials.json')` resolves the filename through the directory, always finding the current inode. Claude Code CLI refreshes the token on the host (new inode); token-sync.js picks up the change on its next 60s cycle.

**Persistent Agent State (REQ-28):** The `/home/node/.openclaw/` directory inside the gateway container is mounted via hostPath to `/home/ocl-local/openclaw-state/` on the local SSD. This ensures agent sessions (conversation history in `agents/*/sessions/*.jsonl`), accumulated memory (`workspace-*/MEMORY.md`), and workspace state (`workspace-*/.openclaw/workspace-state.json`) all survive pod restarts. Without this mount, `.openclaw` lives in the ephemeral container layer and is wiped on every restart — agents lose all conversation context and accumulated knowledge. The `init-permissions` initContainer (runs as root) chowns the directory to uid 1000 before the main container starts. The startup script uses idempotent patterns: `mkdir -p` (no-op on existing dirs), `if [ ! -f ]` guards (preserve existing workspace-state and memory), and `cp` for config/SOULs (always picks up ConfigMap changes). Each agent workspace is seeded with a `MEMORY.md` containing an identity baseline (name, role, owner, network facts) on first boot only — subsequent restarts preserve whatever the agent has learned.
...
2. **Installs the new binary on the host** via `npm install -g openclaw@$TARGET` (the container runs from the hostPath-mounted global module, not from an env var)
3. Re-clears Anthropic SDK shim patches (new SDK version may have different code structure)
4. Checks skill compatibility in both `templates/skills/` and `skills/` directories (template.yaml and SKILL.md formats)
5. Updates the pin in `state.yaml`
6. Pre-flight lock: pauses all Redis task queues cluster-wide
7. Rolling restart: upgrades Home gateway first, health checks, then Cloud, then GPU
8. If any gateway fails health check → automatic rollback to previous version
9. Post-upgrade: advises re-running the wizard for full artifact regeneration

**Important:** `ocl-upgrade` updates the binary and restarts pods, but does NOT regenerate SOULs, OpenClaw config JSON, hooks config, or NAS skills. These artifacts are generated by the wizard and persist in ConfigMaps and hostPath volumes. For a complete upgrade including all artifacts, re-run `bash setup-wizard.sh` after `ocl-upgrade` — the wizard detects the existing install and refreshes everything.
...
The wizard and `ocl-upgrade` validate these constraints before deploying. Skills are checked in both `~/ocl-deploy/templates/skills/` (legacy template.yaml format) and `~/ocl-deploy/skills/` (SKILL.md format). If a skill requires OCL ≥1.5 but you're pinned to 1.4.2, the upgrade warns and refuses to proceed.
