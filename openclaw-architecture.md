...
**UID Mismatch Workaround:** The `ocl` host user is uid 1001, but the gateway pod's `node` user is uid 1000. The credentials file must be `chmod 644` (owner rw, others r) for the pod to read it. Both `token-sync.js` and the CronJob write with `{ mode: 0o644 }` to preserve readability across UIDs. The wizard sets this during `deploy_gateway_pod()`.
